...
class CashierForm extends Component
{
    ...
    protected $listeners = ['editCashierData'];
    
    public function render()
    {
        return view('livewire.admin-panel.manage-accounts.cashier-form');
    }

    public function editCashierData($UserID)
    {
        $this->UserID=$UserID;
        ...
        try {
            if($this->UserID){
                ...
                $log_data = ([
                    ...
                    'activity'      =>  'This Cashier ID. '.($this->UserID).' is successfully Updated to Cashier Table',
                ...
                $log_data = ([
                    'user_id'       =>  Auth::user()->id,
                    'activity'      =>  'This Cashier ID. '.($show['id']).' is successfully Store to Cashier Table',
                    'created_at'    =>  date('Y-m-d H:i:s')
                    ]);
                UserActivityLogsDatabase::create($log_data);
    ...
    public function closeCashierForm(){
        $this->emit('closeCashierModal');
        $this->emit('refresh_admin_table');
        $this->reset();
        $this->resetErrorBag();
