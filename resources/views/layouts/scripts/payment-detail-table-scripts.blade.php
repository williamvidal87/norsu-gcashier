...
@push('payment-detail-table-scripts')
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function () {
        ...
        window.livewire.on('openPaymentDetailModal', () => {
            $('#paymentdetailModal').modal('show')
        });
        
        window.livewire.on('closePaymentDetailModal', () => {
            $('#paymentdetailModal').modal('hide')
        });
        
        
        
        window.livewire.on('openSwalDelete', (PaymentDetailID) => {
            swal("Are you sure you want to delete this Data?", {
                dangerMode: true,
                buttons: true,
            }).then(okay => {
                if (okay) {
                    window.Livewire.emit('DeleteData', PaymentDetailID)
                }else{
                    window.Livewire.emit('refresh_paymentdetail_table')
                }
                });
        });
