...
@push('mode-of-payment-table-scripts')
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function () {
        ...
        window.livewire.on('openModeOfPaymentModal', () => {
            $('#modeofpaymentModal').modal('show')
        });
        
        window.livewire.on('closeModeOfPaymentModal', () => {
            $('#modeofpaymentModal').modal('hide')
        });
        
        
        
        window.livewire.on('openSwalDelete', (ModeOfPaymentID) => {
            swal("Are you sure you want to delete this Data?", {
                dangerMode: true,
                buttons: true,
            }).then(okay => {
                if (okay) {
                    window.Livewire.emit('DeleteData', ModeOfPaymentID)
                }else{
                    window.Livewire.emit('refresh_modeofpayment_table')
                }
                });
        });
