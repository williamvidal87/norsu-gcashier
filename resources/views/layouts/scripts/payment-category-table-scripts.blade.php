...
@push('payment-category-table-scripts')
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function () {
        ...
        window.livewire.on('openPaymentCategoryModal', () => {
            $('#paymentcategoryModal').modal('show')
        });
        
        window.livewire.on('closePaymentCategoryModal', () => {
            $('#paymentcategoryModal').modal('hide')
        });
        
        
        
        window.livewire.on('openSwalDelete', (PaymentCategoryID) => {
            swal("Are you sure you want to delete this Data?", {
                dangerMode: true,
                buttons: true,
            }).then(okay => {
                if (okay) {
                    window.Livewire.emit('DeleteData', PaymentCategoryID)
                }else{
                    window.Livewire.emit('refresh_paymentcategory_table')
                }
                });
        });
